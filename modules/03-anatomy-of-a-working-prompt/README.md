...
- **Tier 1 (required-output checklist):** prompt text contains ≥3 of 4 literal labeled markers (`Role:`, `Task:`, `Format:`, `Constraint:`); references ≥2 named resources by exact filename; confirmed (self-attested) saved, scheduled, and shared; **real, run-dependent evidence:** two actual Copilot output transcripts - one from the crafted prompt, one from a supplied naive prompt, both run against the same data - where the crafted-prompt transcript contains a named structural element absent from the naive one's real output. This can't be satisfied by hand-writing conforming text.
...
The core prompt template - the spine every later module's prompt pack extends. Packaged by Coachgremlin once the gate is met.
